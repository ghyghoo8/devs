...
// Module status:
//  1. downloaded - The script file has been downloaded to the browser.//文件已经被下载
//  2. define()d - The define() has been executed. //内部define方法被执行
//  3. memoize()d - The module info has been added to memoizedMods.//模块信息注册成功
//  4. require()d -  The module.exports is available. //导出模块可用


/**
 * The private data. Internal use only.内部使用
 */
seajs._data = {
...
/**
 * The private utils. Internal use only.内部使用
 */
seajs._util = {};


/**
 * The inner namespace for methods. Internal use only.内部使用
 */
seajs._fn = {};

/**
 * @fileoverview The minimal language enhancement.  编程扩展，类似于underscore.js的原生方法扩展,存放于 seajs._util 中
 */
...
/**
 * @fileoverview The error handler. //错误处理
 */
...
(function(util, data) {
     ...
     *
     * @param {Object} o The error object. //传入错误对象
     */
...
(function(util, data, global) {
    ...
    /**
     * Extracts the directory portion of a path. 摘录目录部分作为路径，正则提取
    ...
    /**
     * Parses alias in the module id. Only parse the prefix and suffix.-->只是解析前缀和后缀
     */
    ...
    /**
     * Caches mod info to memoizedMods. //缓存mod信息到mod集合中
     * defined的回调函数中，第三个参数(mod对象)，包含id和该mod的export对象
     */
    function memoize(id, url, mod) {
        ...
        mod.dependencies = ids2Uris(mod.dependencies, uri);//依赖性，？模块依赖id或者url，唯一标识
    ...
    /**
     * Set mod.ready to true when all the requires of the module is loaded.//模块被加载完成后，设置为true
     */
    ...
    util.getScriptAbsoluteSrc = function(node) {
        return node.hasAttribute ? // non-IE6/7
            ...
            // see http://msdn.microsoft.com/en-us/library/ms536429(VS.85).aspx
                node.getAttribute('src', 4);//4 Returns attribute value as a fully expanded URL.
    };
 ...
 *
 * defined回调函数的第一个参数(require()),可以继续请求模块
 */
...
(function(util, data, fn, global) {
    ...
    fn.load = function(ids, callback, refUrl) {
        ...
        provide(uris, function() {
            ...
            if (callback) {
                callback.apply(global, args);//global===this,this.seajs=seajs
            }
        });
    };
    ...
    function provide(uris, callback) {
        ...
        function onProvide() {
            util.setReadyState(unReadyUris);//设置为已经完成
            callback();//执行回调函数
        }
    }
    ...
    function getUrl(uri) {
        ...
        if (data.config.debug == 2) {
            url = util.addNoCacheTimeStamp(url);//url尾部加时间戳
        }
...
(function(util, data, fn) {
    ...
    function createRequire(sandbox) {
        ...
        //define中，require函数，下挂一个公共函数 require.async
        function require(id) {
